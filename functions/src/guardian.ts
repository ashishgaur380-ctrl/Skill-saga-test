...
export const getLinkedLearnerProgress=onCall(async r=>{
 ...
 const attempts=a.docs.map(d=>{const x=d.data();return{id:d.id,quizId:text(x.quizId),correct:Number(x.correct)||0,total:Array.isArray(x.answers)?x.answers.length:Number(x.total)||0,percentage:Number(x.percentage)||0,createdAt:x.createdAt??null};}).sort((x,y)=>String(y.createdAt?.toMillis?.()??"").localeCompare(String(x.createdAt?.toMillis?.()??""))).slice(0,20);
 return {learner:{id:learnerId,email:text(q.data()?.email),displayName:text(q.data()?.displayName)},stats:{xp,coins,level:Math.max(1,Math.floor(xp/100)+1),attempts:a.size,correct,answered,accuracy:answered?Math.round(correct/answered*10000)/100:0},attempts};
});