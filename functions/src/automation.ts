...
export const listAutomationRules=onCall({cors:true},async r=>{auth(r);try{const s=await getFirestore().collection("automationRules").limit(500).get();const items=s.docs.map(d=>({id:d.id,...d.data()}));items.sort((a:any,b:any)=>String(a.name??"").localeCompare(String(b.name??"")));return{items};}catch(error:any){console.error("listAutomationRules failed",error);const message=error instanceof Error?error.message:String(error??"Unknown Firestore error");throw new HttpsError("internal",`Unable to load automation rules: ${message}`);}});
export const createAutomationRule=onCall({cors:true},async r=>{const{uid,role}=auth(r);const data=validate((r.data as any)?.data??{});const db=getFirestore(),ref=db.collection("automationRules").doc();await ref.set({...data,createdBy:uid,updatedBy:uid,createdAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp()});await db.collection("auditLogs").doc().set(audit(uid,role,"CREATE",ref.id));return{id:ref.id};});
export const updateAutomationRule=onCall({cors:true},async r=>{const{uid,role}=auth(r);const p=r.data as any,id=text(p?.id);if(!id)throw new HttpsError("invalid-argument","id is required.");const data=validate(p?.data??{});const db=getFirestore(),ref=db.collection("automationRules").doc(id);if(!(await ref.get()).exists)throw new HttpsError("not-found","Automation rule was not found.");await ref.update({...data,updatedBy:uid,updatedAt:FieldValue.serverTimestamp()});await db.collection("auditLogs").doc().set(audit(uid,role,"UPDATE",id));return{success:true};});
export const setAutomationEnabled=onCall({cors:true},async r=>{const{uid,role}=auth(r);const p=r.data as any,id=text(p?.id);if(!id)throw new HttpsError("invalid-argument","id is required.");const enabled=Boolean(p?.enabled);const db=getFirestore(),ref=db.collection("automationRules").doc(id);if(!(await ref.get()).exists)throw new HttpsError("not-found","Automation rule was not found.");await ref.update({enabled,active:enabled,updatedBy:uid,updatedAt:FieldValue.serverTimestamp()});await db.collection("auditLogs").doc().set(audit(uid,role,enabled?"ENABLE":"DISABLE",id));return{success:true};});
...
export const runAutomationEngineNow=onCall({cors:true},async r=>{
  auth(r);
  const db=getFirestore();
  const snap=await db.collection("automationRules").where("active","==",true).where("enabled","==",true).limit(500).get();
  const now=new Date();
