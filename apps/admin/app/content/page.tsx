...
export default function ContentPage(){
 ...
 async function selectBoard(id:string){set("boardId",id);set("classId","");set("subjectId","");set("chapterId","");set("topicId","");setChapters([]);setTopics([]);try{const s=await academic("subjects",token);setSubjects(s.filter((x:A)=>(x as any).boardIds?.includes(id)||!(x as any).boardIds));}catch(e:any){setMsg(e.message)}}
 ...
 <section className="panel"><h2>Bulk Import</h2><p className="muted">CSV/XLSX/XLS · up to 500 rows. Required column: <b>title</b>. Optional: description, type, boardId, classId, subjectId, chapterId, topicId, language, accessType, status, fileUrl, thumbnailUrl, publishAtMs, expireAtMs.</p><div className="actions"><input type="file" accept=".csv,.xlsx,.xls,.txt" onChange={e=>setBulkFile(e.target.files?.[0]||null)}/><button disabled={!bulkFile||busy} onClick={()=>void bulkImport()}>{busy?"Processing…":"Import Content"}</button></div></section>
 ...
 <label>Type<select value={form.type} onChange={e=>set("type",e.target.value)}><option value="pdf">PDF</option><option value="video">Video</option><option value="article">Article</option><option value="link">Link</option></select></label>
 ...
 <label>Class<select value={form.classId} onChange={e=>set("classId",e.target.value)}><option value="">All classes</option>{classes.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></label>
 <label>Subject<select value={form.subjectId} onChange={e=>void selectSubject(e.target.value)}><option value="">All subjects</option>{subjects.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></label>
 <label>Chapter<select value={form.chapterId} onChange={e=>void selectChapter(e.target.value)}><option value="">All chapters</option>{chapters.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></label>
 <label>Topic<select value={form.topicId} onChange={e=>set("topicId",e.target.value)}><option value="">All topics</option>{topics.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></label>
 ...
 <section className="panel"><h2>Content library</h2>{items.map(i=><article className="panel" key={i.id}><strong>{i.title}</strong><p>{i.type} · {i.status} · {i.boardId||"All boards"} · {i.classId||"All classes"} · {i.subjectId||"All subjects"}</p><p>{i.description}</p>{i.fileUrl&&<a href={i.fileUrl} target="_blank" rel="noreferrer">Open file</a>}{" "}<button onClick={()=>{setEditing(i.id!);setForm(i)}}>Edit</button>{" "}<button onClick={()=>void archive(i.id!)}>Archive</button></article>)}</section>
 </section></main>;
}
