...
type Mode="quiz"|"topic"|"competition";
export default function Play(){
 ...
 const [q,setQ]=useState(0),[answers,setAnswers]=useState<number[]>([]),[result,setResult]=useState<any>(null),[showReview,setShowReview]=useState(false),[token,setToken]=useState(""),[error,setError]=useState("");
 useEffect(()=>onAuthStateChanged(learnerAuth,async u=>{
   if(!u)return;
   const t=await u.getIdToken();setToken(t);
   try{
     const params=new URLSearchParams(window.location.search);
     const topicId=params.get("topicId")||"", competitionId=params.get("competitionId")||"", quizId=params.get("quizId")||"";
     if(topicId){setMode("topic");setContextId(topicId);await startTopic(topicId,t);}
