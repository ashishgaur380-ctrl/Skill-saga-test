...
export default function AttemptDetails() {
  ...
  useEffect(() => onAuthStateChanged(learnerAuth, async user => {
    if (!user || !params?.attemptId) return;
    try {
      setData(await learnerFunction("getLearnerAttemptDetails", { attemptId: params.attemptId }, await user.getIdToken()));
    } catch (e) {
      ...
      <main className="ss-main">
        ...
        {data?.questions?.map((q:any) => (
          <section className="ss-card" key={q.questionId}>
            <div className="ss-question-head">
            ...
            <div className="ss-review-options">
              {q.options.map((option:string, index:number) => {
                ...
                return (
                  <div key={index} className={selected && correct ? "ss-review-option correct" : selected ? "ss-review-option selected" : correct ? "ss-review-option answer" : "ss-review-option"}>
