...
export default function Home(){
 const [name,setName]=useState("Aarav"),[s,setS]=useState({level:5,xp:320,coins:850,streak:7,quizzes:18,badges:3});
 useEffect(()=>onAuthStateChanged(learnerAuth,async u=>{if(!u)return;setName(u.displayName?.split(" ")[0]||"Aarav");try{const x=await learnerFunction("getLearnerHome",{},await u.getIdToken());if(x?.stats)setS(v=>({...v,...x.stats}));}catch{}}),[]);
 ...
 <section className="ss-mission"><div className="ss-target">🎯</div><div><small>TODAY&apos;S MISSION</small><h2>Complete a quiz today</h2><p>5 questions · Easy</p><Link href="/play" className="ss-primary">Start Now →</Link></div><span>✨</span></section>
 <div className="ss-quick"><Link href="/play">📝<b>Daily Quiz</b></Link><Link href="/play">🏆<b>Weekly Quiz</b></Link><Link href="/learn">📚<b>Learn</b></Link><Link href="/compete">🏅<b>Compete</b></Link></div>
