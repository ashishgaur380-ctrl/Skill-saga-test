...
export default function LearnerHome(){
 ...
 <section className="ss-card ss-wide" style={{marginBottom:14}}><span className="ss-eyebrow">Today&apos;s learning</span><h2>{home?.dailyQuiz?"Daily Quiz is ready":"Keep your learning streak going"}</h2><p>{home?.dailyQuiz?.description||"Complete a quiz to build XP, coins and knowledge."}</p><div className="ss-actions">{home?.dailyQuiz?<Link className="ss-btn primary" href={`/play?quizId=${home.dailyQuiz.id}`}>Start Daily Quiz</Link>:<Link className="ss-btn primary" href="/play">Choose a Quiz</Link>}{home?.weeklyQuiz&&<Link className="ss-btn" href={`/play?quizId=${home.weeklyQuiz.id}`}>Weekly Quiz</Link>}</div></section>
 ...
 <section style={{marginTop:20}}><div className="ss-card"><span className="ss-eyebrow">Rewards</span><h2>Turn learning into rewards</h2><p>Use your earned coins to explore the reward catalogue.</p><div className="ss-actions"><Link className="ss-btn primary" href="/rewards">Open Rewards</Link></div></div></section><section style={{marginTop:20}}><h2>Available quizzes</h2><div className="ss-grid">{(home?.featuredQuizzes||[]).map(q=><Link href={`/play?quizId=${q.id}`} className="ss-card" key={q.id}><span className="ss-eyebrow">{q.questionCount} questions</span><h3>{q.title}</h3><p>{q.description||"Start learning and test yourself."}</p></Link>)}{!home?.featuredQuizzes?.length&&<div className="ss-card"><p>No additional published quizzes are available yet.</p></div>}</div></section>
 </main><Nav/></div>;
}