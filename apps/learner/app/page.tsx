...
export default function Home(){
 ...
 useEffect(()=>onAuthStateChanged(learnerAuth,async u=>{if(!u){window.location.href=APP_BASE+"/login/";return;}setName(u.displayName?.split(" ")[0]||"Aarav");try{const x=await learnerFunction("getLearnerHome",{},await u.getIdToken());if(x?.stats)setS(v=>({...v,...x.stats}));
     try{const stats=await learnerFunction("getLearnerStats",{},await u.getIdToken());if(stats?.stats)setS(v=>({...v,...stats.stats,quizzes:Number(stats.stats.attempts)||0,badges:0}));}catch{} try{const p=await learnerFunction("getLearnerProgress",{},await u.getIdToken()); setContinueTopic((p?.topics||[])[0]||null);}catch{}if(x?.settings)setHomeSettings(v=>({...v,...x.settings}));setDailyQuizId(x?.dailyQuiz?.id||"");setWeeklyQuiz(x?.weeklyQuiz||null);setWeeklyQuizId(x?.weeklyQuiz?.id||"");}catch{}}),[]);
