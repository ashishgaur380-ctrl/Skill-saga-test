...
<div className="ss-subjects">{subjects.map(([icon,title,count])=><Link href={"/learn?subject="+encodeURIComponent(title)} data-practice-href={"/play?topicId="+encodeURIComponent(title)} key={title}><span>{icon}</span><div><b>{title}</b><small>{count}</small></div><strong>›</strong></Link>)}</div>
{materials.length>0&&<section className="ss-library"><div className="ss-heading"><b>Learning Library</b><span>New</span></div>{materials.slice(0,3).map(m=><a href={m.fileUrl||"#"} target="_blank" rel="noreferrer" key={m.id}><b>{m.title}</b><small>{m.type} · {m.language}</small><span>Open {m.type.toUpperCase()}</span>›</a>)}</section>}
