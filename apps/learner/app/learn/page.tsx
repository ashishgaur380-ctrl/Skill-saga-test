...
export default function Learn(){
 ...
 return <div className="ss-shell"><header className="ss-top"><div className="ss-wrap"><div className="ss-brand">SKILL SAGA · LEARN</div><h1>Learn</h1><p>Boards → Classes → Subjects → Chapters → Topics</p></div></header><main className="ss-main">
 {error&&<section className="ss-card" style={{marginBottom:14}}><p>{error}</p></section>}
 <section className="ss-card"><span className="ss-eyebrow">1 · Board</span><div className="ss-actions">{boards.map(b=><button key={b.id} className={selectedBoard?.id===b.id?"ss-btn primary":"ss-btn"} onClick={()=>chooseBoard(b)}>{b.name}</button>)}</div></section>
 {selectedBoard&&<section className="ss-card" style={{marginTop:14}}><span className="ss-eyebrow">2 · Class</span><div className="ss-actions">{classes.map(c=><button key={c.id} className={selectedClass?.id===c.id?"ss-btn primary":"ss-btn"} onClick={()=>chooseClass(c)}>{c.name}</button>)}</div></section>}
 {selectedClass&&<section className="ss-card" style={{marginTop:14}}><span className="ss-eyebrow">3 · Subject</span><div className="ss-grid" style={{marginTop:12}}>{subjects.map(s=><button key={s.id} className="ss-card" style={{textAlign:"left",cursor:"pointer"}} onClick={()=>chooseSubject(s)}><h3>{s.name}</h3><p>{s.code||"Subject"}</p></button>)}</div></section>}
 {selectedSubject&&<section className="ss-card" style={{marginTop:14}}><span className="ss-eyebrow">4 · Chapter</span><div className="ss-grid" style={{marginTop:12}}>{chapters.map(c=><button key={c.id} className="ss-card" style={{textAlign:"left",cursor:"pointer"}} onClick={()=>chooseChapter(c)}><h3>{c.name}</h3></button>)}</div></section>}
 {selectedChapter&&<section className="ss-card" style={{marginTop:14}}><span className="ss-eyebrow">5 · Topics</span>{topics.length?<div className="ss-grid" style={{marginTop:12}}>{topics.map(t=><div key={t.id} className="ss-card"><h3>{t.name}</h3><p>Learning content and practice will connect here.</p></div>)}</div>:<p style={{marginTop:12}}>No topics have been published for this chapter yet.</p>}<div className="ss-actions"><Link href="/play" className="ss-btn primary">Practice with Quiz</Link></div></section>}
 </main><Nav/></div>;
}